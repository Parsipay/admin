...
class biiq_Browsers
{
    ...
    const BROWSER_PARSIPAY_CLIENT = 'Parsipay_Client'; // https://parsipay.org/
    const BROWSER_PARSIPAY = 'Parsipay'; // https://parsipay.org/
    ...
    /**
     * Determine if the browser is Parsipay App or not
     * @return boolean True if the browser is ParsipayApp otherwise false
     */
    protected function checkBrowserParsipay(){
        if (stripos($this->_agent, 'parsipay_client') !== false) {
            $aresult = explode('/', stristr($this->_agent, 'parsipay_client'));
            if (isset($aresult[1])) {
                $aversion = explode(' ', $aresult[1]);
                $this->setVersion($aversion[0]);
                $this->setBrowser(self::BROWSER_PARSIPAY_CLIENT);
                return true;
            }
        }
        if (stripos($this->_agent, 'parsipay') !== false) {
            $aresult = explode('/', stristr($this->_agent, 'parsipay'));
            if (isset($aresult[1])) {
                $aversion = explode(' ', $aresult[1]);
                $this->setVersion($aversion[0]);
                $this->setBrowser(self::BROWSER_PARSIPAY);
                return true;
            }
        }
        return false;
